            ...
            <tbody>
                @foreach ($orders as $order)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">{{ count($order->orderItems) * $item->quantity}}</td>
                        <td class="px-4 py-2">
                            <a href="{{route('invoice', $order)}}" class="bg-orange-500 text-white px-4 py-2 font-semibold rounded-full">View
                                Details</a>
                        </td>
                    </tr>
                @endforeach
